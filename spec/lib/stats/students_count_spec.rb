...
describe StudentsCount do
  ...
  describe ".calculate" do
    ...
    context "with teacher_name option" do
      specify{ StudentsCount.calculate(teacher_name: 'teacher-1').should == 6 }
    end
    ...
    context "with :year option" do
      ...
      before do
        ...
        quick_create 1, 'student', in_account: history_account, st_until: Date.civil(2013,1,1)
        quick_create 1, 'student', in_account: history_account, st_until: Date.civil(2012,12,1)
        quick_create 1, 'student', in_account: history_account, st_until: Date.civil(2012,11,1)

      end

      it "returns count at the last day of the year" do
        StudentsCount.calculate(year: 2012, account_name: history_account.name).should == 1
      end

      context "and :month option" do
        it "returns count at the last day of the month" do
          StudentsCount.calculate(year: 2012, month: 11, account_name: history_account.name).should == 2
        end
      end
    end
    context "with :month option" do
      it "returns average for such month"
    end
  end

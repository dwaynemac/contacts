...
class NewImport < ActiveRecord::Base
  self.table_name = "imports"

  serialize :failed_rows, Array
  serialize :headers, Array

  has_many :contact_imports, foreign_key: :import_id
  has_many :imported_contacts, through: :contact_imports, class_name: "NewContact", :source => :contact

  belongs_to :account, :class_name => "NewAccount"

  belongs_to :attachment, :class_name => "NewAttachment" #, cascade_callbacks: true
          ...
          if contact_exists?(remote_id)
            ...
            if contact.valid?
              if contact.save
                ...
                self.imported_contact_ids << contact.id
              end
            else
  ...
  def build_contact(row)
    @current_row = row
    @contact = NewContact.new(owner_id: self.account.id)
  ...
  private
    ...
    def destroy_all_imported_documents
      self.imported_contacts.destroy_all
    end
    ...
    def contact_exists?(kshema_id)
      !NewContact.where(kshema_id: kshema_id).empty?
    end
